...
module.exports.getaOlympicGamesbycities = function (req,res){
	var name = req.params.city
	console.log("New GET of resource "+name+ " from "+req.connection.remoteAddress);
	var searched = search.searchByparam(olympicsgames,"city",name)
	if(searched==""){
...
//PUT
 ...
 //console.log("composer1 "+req.body.hasOwnProperty("composer"))
 if(checkvar== true){
 	var searched = searchByparam(olympicsgames,'city',name)
...
function searchByparams(comp,param){
	console.log(comp)
	var searched = olympicsgames.filter(function (content) {
    	return content.param.toLowerCase() == comp.toLowerCase();});
	return searched;};
